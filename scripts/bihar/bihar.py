...
def extract_first_page_details(path):
    ...
    if len(text) == 10:
        main_town,police_station,revenue_division,mandal,district,pin_code = split_data(text[0]),split_data(text[2]),str(split_data(text[3])),split_data(text[7]),split_data(text[8]),split_data(text[9]),
    else:
...
if __name__ == '__main__':
    ...
    a_pool = multiprocessing.Pool(60)
