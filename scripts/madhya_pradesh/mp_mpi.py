...
def extract_first_page_details(path,input_images_blocks_path):
	...
	if len(text) == 7:
		main_town,police_station,revenue_division,mandal,district,pin_code = split_data(text[0],seps),split_data(text[3],seps),split_data(text[2],seps),split_data(text[4],seps),split_data(text[5],seps),split_data(text[6],seps)
	else:
	...
	if len(text) == 2:
		...
		ac_name,parl_constituency = split_data(ac_name,seps),split_data(text[1],seps)
	else:
